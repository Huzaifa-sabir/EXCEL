...
.container{max-width:900px;margin:0 auto;background:#fff;border-radius:20px;padding:40px}
.form-row{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:20px}
input,select{width:100%;padding:12px;border:2px solid #e0e0e0;border-radius:8px}
button{background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;border:none;padding:14px 28px;border-radius:8px;cursor:pointer;margin-right:10px}
.card{background:#f8f9fa;border-radius:12px;padding:20px;margin:10px 0;border-left:4px solid #667eea;display:flex;justify-content:space-between}
...
<div><label>Passport</label><input id="passport"></div>
</div>
...
c.push({firstName:f,lastName:l,category:document.getElementById('category').value,subcategory:document.getElementById('subcategory').value,agentName:a,passport:document.getElementById('passport').value});
document.getElementById('cnt').textContent=c.length;
document.getElementById('list').style.display='block';
document.getElementById('cust').innerHTML=c.map((x,i)=>`<div class="card"><div><b>${x.firstName} ${x.lastName}</b><br>${x.category}</div><button class="remove" onclick="c.splice(${i},1);add()">Remove</button></div>`).join('');
...
def generate():
    try:
        ...
        # Add customers
        for c in customers:
            ds.append(['Bissau',c.get('category','National Visa'),c.get('subcategory','Work Visa'),'',c.get('lastName',''),c.get('firstName',''),c.get('passport',''),'','','Male','','GUINEA-BISSAU','','',c.get('agentName',''),'','',''])
